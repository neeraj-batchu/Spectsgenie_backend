...
router.post('/create-order', async (req, res) => {
    const { name, mobileNumber, amount, orderData, addressId, userId,address } = req.body;
    ...
    try {
        ...
        await db.query(
            `INSERT INTO sg_order_transactions (transactionId, code, providerReferenceId, order_id, amount)
             VALUES (?, ?, ?, ?, ?)`,
            [transactionDetails.transactionId, transactionDetails.code, transactionDetails.providerReferenceId, transactionDetails.order_id, transactionDetails.amount]
        );
        ...
        const orderDetails = {
            ...
            discount_code: null, // Assuming no discount code
        ...
        await db.query(
            `INSERT INTO sg_orders_online
             (id, order_id, customer_id, address_id, total_amount, actual_total_amount, discount, discount_code, order_status, orderData)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`,
            [
                0,
...
`).join('')}
        ...
        const vendorPhoneNumber = '9701090085'; // Replace with the vendor's phone number
